...
class RAGflowClient:
    ...
    def update_dataset(self, dataset_id: str, name: Optional[str] = None,
        ...
        data = {k: v for k, v in
                {"name": name, "embedding_model": embedding_model, "chunk_method": chunk_method}.items() if
                v is not None}
    ...
    def update_document(self, dataset_id: str, document_id: str, name: Optional[str] = None,
        ...
        data = {k: v for k, v in {"name": name, "chunk_method": chunk_method, "parser_config": parser_config}.items() if
                v is not None}
        response = requests.put(url, json=data, headers=self.headers)
        if not response.ok:
    ...
    def update_chunk(self, dataset_id: str, document_id: str, chunk_id: str,
        ...
        data = {k: v for k, v in
                {"content": content, "important_keywords": important_keywords, "available": available}.items() if
                v is not None}
        response = requests.put(url, json=data, headers=self.headers)
        if not response.ok:
    ...
    def update_chat(self, chat_id: str, name: Optional[str] = None, avatar: Optional[str] = None,
        ...
        data = {k: v for k, v in
                {"name": name, "avatar": avatar, "dataset_ids": dataset_ids, "llm": llm, "prompt": prompt}.items() if
                v is not None}
        response = requests.put(url, json=data, headers=self.headers)
        if not response.ok:
