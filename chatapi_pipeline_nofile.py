...
class Pipeline:
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        if self.debug:
            ...
            if self.sessionKV.get(chat_id):
                ...
                #创建session
                session_url = f"{self.valves.HOST}:{self.valves.PORT}/api/v1/chats/{chat_id}/sessions"
                session_headers = {
    ...
    def pipe(
        ...
        # print(messages)
        question_url = f"{self.valves.HOST}:{self.valves.PORT}/api/v1/chats/{chat_id}/completions"
        question_headers = {
            'content-Type': 'application/json',
            'Authorization': 'Bearer '+self.valves.API_KEY
        }
